...
void DisegniSegmenter::setInputImage(Mat i) {
    inputImage_ = i;
}



//Conducts all the private class functions to keep the ImageSegmentationMain.cpp concise
...
The tool treats the image like a topographic map, with the brightness of each point representing its height.
The tool then finds the lines that run along the ridges/boundaries of the points of interest.
Finally, it outputs an image with only the points of interest visible.
 */
