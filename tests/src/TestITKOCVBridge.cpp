...
template<typename ITKPixelValueT, typename CVPixelValueT>
void ComparePixel(const itk::RGBPixel<ITKPixelValueT>& itk, const cv::Vec<CVPixelValueT, 3>& cv) {
    for(int d = 0; d < 3; d++) {
        // ITK -> RGB
        // OpenCV -> BGR
...
template<typename ITKPixelValueT, typename CVPixelValueT>
void ComparePixel(const itk::RGBAPixel<ITKPixelValueT>& itk, const cv::Vec<CVPixelValueT, 4>& cv) {
    for(int d = 0; d < 4; d++) {
        // ITK -> RGB
        // OpenCV -> BGR
...
TEST_P(ITKOCVBridge, RoundTrip)
{
    ...
    switch(GetParam()) {
        ...
        case CV_8UC3:
            ...
            break;
        case CV_8UC4:
            // cv::randu(img, cv::Scalar{0, 0, 0, 0}, cv::Scalar{256, 256, 256, 256});
        ...
        case CV_16UC1:
            ...
            break;
        case CV_16UC3:
            // cv::randu(img, cv::Scalar{0, 0, 0}, cv::Scalar{65536, 65536, 65536});
            TestITKConversion<Image16UC3, cv::Vec<uint16_t, 3>>(img, dim);
            break;
        case CV_16UC4:
            // cv::randu(img, cv::Scalar{0, 0, 0, 0}, cv::Scalar{65536, 65536, 65536, 65536});
        ...
        case CV_32FC3:
            ...
            break;
        case CV_32FC4:
            // cv::randu(img, cv::Scalar{0, 0, 0, 0}, cv::Scalar{1.0, 1.0, 1.0, 1.0});
